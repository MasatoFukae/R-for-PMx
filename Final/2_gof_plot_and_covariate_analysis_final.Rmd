---
title: "GOF plot and covariate analysis"
...
#### sdtab
```{r example_sdtab, echo=F, message=F}
...
上記の**sdtab**ファイルから、以下のようなGOFプロットを作成することができます。


#### GOFプロット
```{r example_gof_plot, echo=F, message=F}
...
※**sdtab60**ファイル中には、DRUG=1、2のデータが含まれています。

## 演習-1 解答欄
```{r exercize-1_field, fig.height=5, fig.width=10, message=F}
# パッケージ読み込み
library(dplyr)
library(tidyverse)
library(gridExtra)
...
p1 <- sdtab %>% filter(DRUG==1 & DV > 0) %>% ggplot(aes(x=PRED, y=DV)) + geom_point() +  geom_abline(linetype="solid") +
   stat_smooth(se=FALSE, colour="red", linetype="dashed")

p2 <- sdtab %>% filter(DRUG==1 & DV > 0) %>% ggplot(aes(x=TSLD, y=CWRES)) + geom_point() +  
...
## 演習-1 解答例 
<details><summary>解答を確認する</summary><div>
```{r exercize-1_answer, fig.height=5, fig.width=10, message=F}
...
## 視覚的共変量探索 {#sec02}
### 共変量とは?
薬物動態や薬力学に影響を及ぼす要因のことです。以下に共変量の一例を挙げています。
...
### 共変量探索の流れ
以下に、共変量探索を行う際の手順の一例を示しています。
**実際に演習を行っていただく部分は太字**で示しています。
...
NONMEMで共変量探索を実行する前に、まずは収集した患者背景データに対して共変量探索が可能かどうか確認します。
各変数の要約統計量を算出することで、その特徴や傾向を把握することができます。

 - 連続変数（年齢、体重、臨床検査値など）: 平均、標準偏差、データの範囲、分布の形は？
 - 離散変数（性別、人種など）：全体に占める割合は？

例えば、 データの範囲が狭い場合（健康成人を対象とした試験データにおける年齢、臨床検査値など）や、データの全体に占める割合が少ない場合（男性に偏ったデータにおける性別など）は、その変数を共変量候補とするのに適しません。

### 共変量候補の相関関係の確認
次に、共変量候補が互いに独立しているか確認します。
相関が強い共変量を複数同時に同じパラメータに組み込むと、パラメータの推定精度が悪くなる可能性があります。
例えば、体重とBMI、クレアチニンクリアランスと血清クレアチニン値などが相関の強い共変量候補としてあげられます。
あらかじめ共変量候補間の相関を確認し、相関の強い共変量を複数同時にパラメータに組み込まないように注意する必要があります。

```{r example_ggpairs, echo=F, message=F, warning=F}
...
母集団薬物動態解析は、Base modelを検討後、共変量探索を行うプロセスで進められます。
ただし、これはあくまで一例であり、解析のプロセスが前後したり、プロセスを戻って再検討することはあり得ます。
ここでBase modelとは、構造モデル（薬物動態では1コン、2コンなど）、個体間変動の組み入れの有無、残差誤差モデル等を検討し、共変量探索を行う前のモデルとします。

NONMEMで共変量探索を実行する前に、Base modelから算出されるETAの経験ベイズ推定値（Empirical Bayes Estimate、以下EBE）と共変量候補間に相関関係があるか確認します。
これにより、共変量探索前に組み込まれそうな共変量に当たりをつけることができます。
また、ETAのEBEと共変量候補の関係から、共変量として組み込む際の式を検討できます。例えば、比例的な増加か？べき乗的な増加か？頭打ちとなっているか？などをグラフから読み解くことで、モデリングのヒントとなり得ます。

```{r example_ETA_vs_covariate, fig.height=5, fig.width=15, echo=F, message=F}
...
上記の例では、ETA1のEBEとCRCL（クレアチニンクリアランス）に正の相関関係があり、CRCLの増加に対してEBEの増加はやや頭打ちとなっていることがわかります。
ETA6のEBEとWT（体重）の間には強い相関関係は認められません。
ETA7のEBEとHLTH（0=患者、1=健康成人）の関係から、健康成人でEBEが大きい傾向が読み取れます。
...
読み込んだデータが連続変数か離散変数かで作成されるプロットが異なります。
...
```{r example_ggpairs_2, message=F, warning=F}
...
# 以下にRコードを記載してください。
nmdata2 <- nmdata %>% distinct(ID, .keep_all = TRUE) %>% 
   select(CRCL, AGE, MALE, HLTH) %>% 
   mutate(CRCL = ifelse(CRCL < 0, NA, CRCL),
          MALE = as.factor(MALE),
          HLTH = as.factor(HLTH))

ggpairs(data=nmdata2, aes(alpha=0.7, colour=HLTH))
...
## 演習-3
 - Data folder内の**patab61**、**catab61**、**cotab61**、**sdtab61**について
   ...
   - **nmdata**を用いて**DRUG=1**のデータについて、ETA1とCRCLの相関プロットを作成してください。
   - **nmdata**を用いて**DRUG=1**のデータについて、ETA1とHLTHのボックスプロットを作成してください。

※**patab61**、**catab61**、**cotab61**、**sdtab61**ファイル中にはDRUG=1、2のデータが含まれています。
「CRCL」の値が欠損しているID（CRCL=-99）は削除してください。

### 演習-3に用いるデータセットの説明
 - patab61：患者個別パラメータ（ETAのEBE）を出力したファイル
...
## 演習-3 解答欄
```{r exercize-3_field, fig.height=5, fig.width=10, message=F}
# パッケージ読み込み
library(dplyr)
library(tidyverse)
...
## 演習-3 解答例
<details><summary>解答を確認する</summary><div>
```{r exercize-3_answer, fig.height=5, fig.width=10, message=F}
# パッケージ読み込み
library(dplyr)
library(tidyverse)
