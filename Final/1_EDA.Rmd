...
本日の概要になります。EDAとは何かを簡単に例を示しながら説明した後，主に二つのについて，
学んでいきます。
...
この図では用量ごと，投与期ごとの平均の血中濃度推移を個別の濃度推移を併せて，示しています。
このデータは点滴静注後の血中濃度推移なので，該当しないのですが，仮にこれが経口投与後のデータだったとすると
...
# indiv. Mean plot for selected trials
ggplot(data=nm_data_ex %>% filter(DRUG==2 & PN==19 & MDV == 0), 
       aes(x=RNTM, y=CONC)) + 
  geom_line(aes(group=ID), colour="darkgray") +
  stat_summary(colour="red") + stat_summary(geom = "line", colour="red") + 
  facet_grid(PERD~DOSE)  + scale_colour_discrete(name="Dose (mg)") + 
...
```{r data-import-ex0}
# ファイルをcsv形式で読み込みnm_data_ex0として格納
nm_data_ex0 <- read_csv("/cloud/project/Data/PSP4-8-748-s012.csv")

```

ここではもう一つ，便利なスキルを同時に学んでいただきたいと思います。
...
なお，ここではRがデータからデータ形式を推測して読み込んでいます。

```{r data-import-ex1}
# paste0で結合した文字列で指定したファイルをcsv形式で読み込みnm_data_ex1として格納
nm_data_ex1 <- read_csv(paste0(path, "/Data/PSP4-8-748-s012.csv")) 
# pathはpath <- "/cloud/project" として，最初のchunkで指定している。
```

なぜこのような回りくどいことをするかというと，ファイルの読み込みや吐き出しには必ずパスの指定が必要となるため，
パスを指定する作業を最初にまとめて行うことで，コード量を減らすという意図です。
また，仮にフォルダ構造が変わって，ファイルの場所が変わったとしても，冒頭の**path**を書き換えれば，
以下のコードをそのまま利用できます。

ここでは既に読み込んである変数である**path**を利用して，ファイルを読み込みます。以下の様なイメージです。

![データの指定方法](/cloud/project/Data/1_data_handling/f6.png)

なお絶対パスでファイルの場所を指定する場合，心配はいらないのですが，相対パスでファイルの場所を指定する場合には
重要な点として，**自分が今いる場所**と**ファイルの場所**の関係を理解しておく必要があります。
自分がいる場所を確認するには，**getwd()**を使います
...
CとTREATをcharacterとして読み込ませるように指定します。
...
こうすることで，もしデータの型を誤って読み込んでしまう場合に，強制的に型を指定して読み込ませることが出来ます。
...
**head**では一度にたくさんの行を示すと見づらいので，このように最初の数行を出して，意図通りに読み込めているか確認します。
...
データの全体をみたい場合には**View**を使うとspread sheetとしてみることが出来ます。
...
カラム名のみを見たい場合は**names**もしくは**colnames**を用いることが出来ます。

\pagebreak

さらに読み込んだデータのカウント（計測）を行っていきます。
基本的には行数又は列数を計測する関数になります。
これは次のセクションで説明するデータの抜き出し，要約と組み合わせて使うことも多いです。
...
![データの行(row)列(col)](/cloud/project/Data/1_data_handling/f1.png)
...
![データの抜き出しのイメージ](/cloud/project/Data/1_data_handling/f2.png)
...
sliceは頭から数行を取り出すときに使います。

```{r slice-distinct-Ex}
# nm_dataの最初の2行を取り出す
nm_data %>% slice(1, 2)

# nm_dataの最初の3, 4, 5行を取り出す
nm_data %>% slice(3:5)
...
データの要約では，変数ごとにデータを処理するスキルを学びます。
...
データの要約のイメージです。RACEごとにBWの平均値を求めます。

![データの要約のイメージ](/cloud/project/Data/1_data_handling/f3.png)

ここではまずIDの重複をdistinctで削除し，RACEでグループ化し，SummarizeでBW_meanという体重の平均を示す列を作成しています。
以降にグループ化の必要がない場合，ungroupでグループ化を解除します。

```{r group-summarize-Ex}
# nm_dataのIDごとに最初の1行抜き出し，RACEごとにBWの平均値を求める
nm_data %>% distinct(ID, .keep_all = TRUE) %>% group_by(RACE) %>% summarise(BW_mean = mean(WT)) %>% ungroup()
```

なお要約統計量の算出によく使う関数を以下にまとめています。
...
data folder内のPSP4-8-748-s012.csvをnm_data2として読み込み，
  ...
  1. 試験（PN）ごとの被験者数(ID) *ヒント：IDの重複を削除し，PNごとの行数をカウント*
  2. 試験（PN）ごとのMK7655(DRUG=2)の有効血漿中濃度の数（MDV=0の行数）*ヒント：DRUGとMDVの条件でデータを抜き出し，PNごとの行数をカウント*
  3. PN = 1, 2のデータに絞り，試験（PN）ごとかつ用量（DOSE）ごとの背景因子（AGE, WT, CRCL）の平均値 *ヒント：PNの条件でデータを抜き出し，PN，DOSEごとにグループ化し，背景因子の平均の新たな変数を作る*
...
## 1. Number of Subject by PN
nm_data2 %>% distinct(ID, .keep_all = TRUE) %>% count(PN) 

## 2. Number of MK7655 Vaid Concentration Record by PN
nm_data2 %>% filter(MDV == 0 & DRUG == 2) %>% count(PN) 

## 3. Desc. Stats. Grouped by PN and Dose
...
ここでは他のデータの結合，並び替え，整形についてのスキルを紹介します。
  ...
  * **left_join** : データの左側に別のデータを結合(マージする)
    + **重要：byで結合のためのKeyを指定（複数可能）**，Key名が異なる場合は対応関係を記載
...
以下は**left_join**のイメージです。緑の部分は結合する側（右）のデータには対応するデータがない部分です。
逆にオレンジの部分は結合される側（左）のデータには対応するデータがない部分です。
left_joinでは結合後，オレンジの部分はなくなります。
元のデータにあった緑の部分は残り，結合後のデータにはNAが挿入されます。
**inner_join**ではこれら緑・オレンジは全てなくなります。
逆に**full_join**では全て残り，データがないところにはNAが挿入されます。

![データの結合（left_join）のイメージ](/cloud/project/Data/1_data_handling/f4.png)

```{r Join-Ex}
# 結合するデータの読み込み
nm_data_ibw <- read_csv(paste0(path, "/Data/1_data_handling/nm_data_ibw.csv"))
...
データの並び替えのイメージです。何も指定しない場合，カラムの昇順に並び替えられます。
desc()で指定すると降順となります。

![データの並び替え（arrange）のイメージ](/cloud/project/Data/1_data_handling/f5.png)

**select**は必要なカラムを絞ったり，不要なカラムを削ったり，またカラムを並び替えたりする際に使います。
...
データの結合，並び替えについての演習です(ここは時間が足りなければやりません！)
...
data folder内のPSP4-8-748-s012.csvをnm_data2, back_info.csvをback_infoとして読み込み，
  ...
  2. CYP2D6 Phenotypeごとの被験者数を算出してください *ヒント: IDの重複を削除し，CYP2D6ごとに行数を数える*
...
nm_data2 %>% left_join(back_info, by="ID") %>% distinct(ID, .keep_all = TRUE) %>% count(CYP2D6)
...
まずは基本的なggplotのcodingルールを理解し，散布図と折れ線グラフを作成してみます。
  ...
  - **ggplot**: 以降のdefault設定（ex: 用いるデータ，x, yに使うデータ）を定義する
    + データ中の列を使用する場合，aes()を用いる(**重要：aesはデータ内のどの変数を使うかを指定するところ**）
...
**facet_wrap**では基本的には1つの変数でパネルを分ける(複数もできますが見ずらい。。)，
**facet_grid**では二つの変数で縦/横にパネルを分ける場合に用います。

``` {r Fac-Ex}
## facet_wrap
ggplot(data=nm_data_p, aes(x=TIME, y=CONC, colour=factor(MALE))) + geom_line() + facet_wrap(~ID, scales = "free")
#ggsave("facet_wrap.png", width=10, height = 8)
...
#ggsave("facet_grid.png", width=10, height = 8)
...
nm_data_b %>% ggplot(aes(x=factor(PN), y=CRCL)) + geom_boxplot()

## geom_histogram
# nm_dat_bを使い，CRCLのヒストグラムを作成
nm_data_b %>% ggplot(aes(x=CRCL)) + geom_histogram()

# nm_dat_bを使い，PNで色分けしてCRCLのヒストグラムを作成
nm_data_b %>% ggplot(aes(x=CRCL, fill=factor(PN))) + geom_histogram(position = "identity", alpha = 0.5, binwidth = 10)
...
data folder内のPSP4-8-748-s012.csvをnm_data3として読み込み，以下を作図してください。
  ...
  1. PN=2のデータについて，個別濃度推移とその平均推移を重ね合わせた図を薬剤（DRUG）ごとに作成してください *ヒント：PNの条件で絞り込み，ggplotの基本設定を行い，折れ線グラフ（IDごと），要約統計量をプロット，DRUGで分割する*
  2. 全症例のデータについて，体重（WT）とCRCLの相関プロットを近似曲線付きで示してください。*ヒント：IDの重複を削除して，横軸をWT，縦軸をCRCLとして散布図をプロットし，近似曲線を追加する*
...
## Read dataset
nm_data3 <- read_csv(paste0(path, "/Data/PSP4-8-748-s012.csv"), na=".",
                     col_types = cols(C     = col_character(),
                                      TREAT = col_character() 
                                      ))
## Data to plot 
nm_data_ex3 <- nm_data3 %>% filter(PN==2 & MDV==0)
...
## BW-CLCR
nm_data3 %>% filter(CRCL>0) %>% distinct(ID, .keep_all=TRUE) %>% ggplot(aes(x=WT, y=CRCL)) + geom_point(colour="darkgray") + stat_smooth() + theme_bw()
...
ここでは知っていると便利な機能をおまけ的に紹介します。重要ではないので，時間がなければ省略します。

### Tableの整形 flextable

flextableはきれいにTableを作成するパッケージになります。
作成したTableを次の**officer**を利用してそのまま出力して，提示できます。
...
doc <- read_docx() %>%
  ...
  print(target = paste0(path, "/Data/1_data_handling/Officer_Table.docx")) #  word fileとして出力
...
ppt <- read_pptx() %>% 
  ...
  print(target = paste0(path, "/Data/1_data_handling/Officer_Table.pptx")) #  ppt fileとして出力
