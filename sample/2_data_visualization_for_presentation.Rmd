...
### GOFプロットの作成手順
NONMEMで母集団薬物動態解析を実行時に出力したsdtabファイル中の変数を使用して、GOFプロットを作成します。
今回は、前セクションで学んだRコーディング技術を使用してGOFプロットを作成していただきます。

NONMEMでのsdtabファイルの出力方法等については詳細を割愛させていただきますが、以下に示すようなファイルです。
演習-1ではsdtabファイルから必要なデータを抜き出し、GOFプロットを作成していただきます。
...
演習-2では「共変量候補の相関関係の確認（散布図行列）」、「ETA（変量効果）と共変量の相関を確認（散布図、ボックスプロット）」に取り組んでいただきます。
...
また、ETAのEBEと共変量の関係から、共変量として組み込む際の式を検討できます。例えば、比例的な増加か？べき乗的な増加か？頭打ちとなっているか？
